...
- always test both backend and frontend except explicitly asked to test only one

## Laravel Sail Development Environment

This project uses Laravel Sail (Docker). ALL commands must be run through Sail containers, not directly on the host machine:

- Tests: `sail artisan test` (never `php artisan test`)
- Artisan commands: `sail artisan [command]` (never `php artisan [command]`)
- Composer: `sail composer [command]` (never `composer [command]`)
- NPM/Node: `sail npm [command]` or `sail node [command]` (never direct npm/node)
- Database: `sail mysql` or `sail psql` (never direct database connections)
- Tinker: `sail artisan tinker` (never `php artisan tinker`)

Always prefix commands with `sail` to ensure they run in the proper containerized environment with correct dependencies and configuration.
