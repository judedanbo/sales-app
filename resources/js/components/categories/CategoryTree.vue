...
import { ExpandIcon, Minimize2, Search } from 'lucide-vue-next';
...
function filterCategoriesRecursive(categories: Category[], query: string): Category[] {
    ...
    for (const category of categories) {
        const matchesQuery = category.name.toLowerCase().includes(query) || category.description?.toLowerCase().includes(query);

        const filteredChildren = category.children ? filterCategoriesRecursive(category.children, query) : [];
    ...
    <div class="space-y-4">
        ...
        <div class="space-y-1">
            <div v-if="filteredCategories.length === 0" class="py-8 text-center text-muted-foreground">
                <div v-if="searchQuery">No categories found matching "{{ searchQuery }}"</div>
