...
class UpdateProductRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->has('tax_rate')) {
            $this->merge([
                'tax_rate' => round((float) $this->tax_rate, 4),
            ]);
        }
    }
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            // Prevent changing status to discontinued if there are active sales
            if ($this->input('status') === 'discontinued') {
                $product = $this->route('product');
                if ($product && $this->hasActiveSales($product)) {
                    $validator->errors()->add('status', 'Cannot discontinue product with active sales.');
                }
            }

            // Validate that price changes require appropriate permissions
            if ($this->has('unit_price') && ! $this->user()->hasPermissionTo('manage_pricing')) {
                $validator->errors()->add('unit_price', 'You do not have permission to change product pricing.');
            }
        });
    }

    /**
     * Check if product has active sales (placeholder for business logic)
     */
    private function hasActiveSales($product): bool
    {
        // This would check against sales/orders table
        // Return false for now until sales system is implemented
        return false;
    }
}
